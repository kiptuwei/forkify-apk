...
class RecipeView extends View {
  ...
  _errMsg = 'We could not find the item. Please try another one!';
  _succMsg = '';
  ...
  addHandlerUpdateServings(handler) {
    this._parentElement.addEventListener('click', function (evt) {
      ...
      if (+updateTo > 0 && +updateTo < 21) handler(+updateTo);
    });
  }

  addHandlerBookmark(handler1, handler2) {
    this._parentElement.addEventListener('click', function (evt) {
      ...
      this._data.bookmarked ? '-fill' : ''
          ...
          <div class="recipe__ingredients">
            ...
            ${this._data.ingredients
              .map(this.generateMarkupIngredints)
              .join('')}
            </ul>
          </div>
